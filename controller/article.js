...
module.exports = {
  ...
  handleArticleAddPost(req, res) {
    ...
    conn.query(insertSql, body, (err, result) => {
      ...
      if (err)
        return res
          .status(500)
          .send({ status: 500, msg: "文章发表失败,请重试" });
  ...
  handleArticleInfoGet(req, res) {
    ...
    //console.log(req.params)  //req.params用于获取 :id传递过来的参数
  ...
  handleArticleEditGet(req, res) {
    ...
    conn.query(querySql, (err, result) => {
      if (err || result.length !== 1)
        return res
          .status(500)
          .send({ status: 500, msg: "文章获取失败,请重试", data: null });
      console.log(result);
  ...
  handleArticleEditPost(req, res) {
    ...
    conn.query(updateSql, [article, article.id], (err, result) => {
      if (err || result.affectedRows !== 1)
        return res.status(400).send({ status: 400, msg: "修改文章失败, 请重试!", data: null });
      // 修改成功, 像客户端响应结果
